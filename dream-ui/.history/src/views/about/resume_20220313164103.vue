 ...
 * @LastEditTime: 2022-03-13 16:40:45
-->
<script>
      ...
      ResumeApi.getDetail(uuid).then((response) => {
        if (response.code == 0) {
          ...
          _this.form.region =
            ...
            regionItems[2].name;
            debugger
          _this.form.regionVal = regionItems[2].code+"";
          console.log(_this.form.regionVal)
          _this.form.sex = this.form.sex+"";
          _this.fileList = response.data.photos.split(",");
          _this.form.xingZhuo = this.birthToConstellation(this.form.birth);
          _this.form.shengXiao = this.birthToShengxiao(this.form.birth);
        }
      });
    },
    onSubmit() {
    ...
    afterRead(res) {
      ...
      axios({
        ...
        for (let i = 0; i < imgs.length; i++) {
          this.fileList.push({ url: imgs[i].url, isImage: true });
          ps.push(imgs[i].url);
        }
        this.form.photos = ps.join(",");
      });
    },
  },
};
  ...
  <div>
    <van-form>
      <van-cell-group title="基本信息">
        ...
        <van-area title="标题" :area-list="regionList" value="411521" />
