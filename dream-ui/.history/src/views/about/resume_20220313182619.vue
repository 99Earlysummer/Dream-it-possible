 ...
 * @LastEditTime: 2022-03-13 18:26:05
-->
<script>
...
export default {
  ...
  methods: {
    ...
    onPhotosSubmit(){
        ...
        if (imgs) {
          //let newImgs = imgs.map((item) => ({ url: item.url, isImage: true }));
          //this.fileList = this.fileList.concat(newImgs);
          this.form.photos = imgs.map((item) => item.url).join(",");
               console.log("===========================");
    ...
    beforeDelete(res){
      ...
      this.fileItems.splice(this.fileItems.indexOf(res.file),1);
      this.fileList.splice(this.fileItems.indexOf(res.file),1);
    },
    afterRead(res) {
      console.log(res);
      ...
      //     this.fileItems.push(res[i].file);
      //   }
      // } else {
      //   this.fileItems.push(res.file);
      //}
console.log(this.fileList);
    },
  },
