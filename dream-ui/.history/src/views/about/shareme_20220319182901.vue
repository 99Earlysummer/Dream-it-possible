 ...
 * @LastEditTime: 2022-03-19 18:29:01
-->
<script>
...
export default {
  ...
  mounted() {
    ...
    console.clear();
    console.log("%c 山野万里，你是我藏在微风中的欢喜", "background:#24272A; color:#ffffff", "");
    console.log("%c Github %c", "background:#24272A; color:#ffffff", "", "https://github.com/MiracleTanC");
    ...
    >
      ...
      <div class="bg">
       ...
       <van-cell-group title="基本信息">
          <van-cell v-show="form.name" title="姓名" :value="form.name" />
          ...
          <van-cell v-show="form.height" title="身高" :value="form.height+'cm'" />
          <van-cell v-show="form.weight" title="体重" :value="form.weight+'kg'" />
          <van-cell v-show="form.region" title="籍贯" :value="form.region" />
          <van-cell v-show="form.school" title="毕业院校" :value="form.school" />
          <van-cell v-show="form.company" title="就职单位" :value="form.company" />
        </van-cell-group>
      </van-swipe-item>
      <van-swipe-item>
        ...
        <van-cell-group title="有趣的灵魂总是逗比">
           ...
           <ul style="margin:1em">
              <li v-for="(m,index) in photos">
                <img class="vphoto" :src="m" @click="phClick(index)"/>
              </li>
            </ul>
...
<style scoped>
.bg{
  background-image: url('http://file.miaoleyan.com/wall/B6A8D50587009F0F6FD85889232D5F02.png') no-repeat  width:100%;
    height:40%;
...
.subtitle{
   ...
   text-align:center;
   font-size: 1.3em;
   animation: titleText 2s infinite !important;
}
.up-arrow {
  width: 2rem;
  height: 2rem;
